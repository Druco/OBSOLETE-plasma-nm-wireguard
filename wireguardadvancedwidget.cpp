...
void WireGuardAdvancedWidget::wireguardCipherFinished(int exitCode, QProcess::ExitStatus exitStatus)
{
    ...
    if (!exitCode && exitStatus == QProcess::NormalExit) {
        m_ui->cboCipher->addItem(i18nc("@item::inlist Default wireguard cipher item", "Default"));
        const QList<QByteArray> rawOutputLines = d->wireguardCiphers.split('\n');
...
void WireGuardAdvancedWidget::loadConfig()
{
    ...
    const QString wireguardKeyTa = dataMap[QLatin1String(NM_OPENVPN_KEY_TA)];
    const QString wireguardKeyTlsCrypt = dataMap[QLatin1String(NM_OPENVPN_KEY_TLS_CRYPT)];

    if (!wireguardKeyTlsCrypt.isEmpty()) {
        m_ui->cboTLSMode->setCurrentIndex(2); // TLS-Crypt
        m_ui->kurlTlsAuthKey->setUrl(QUrl::fromLocalFile(wireguardKeyTlsCrypt));
    } else if (!wireguardKeyTa.isEmpty()) {
        m_ui->cboTLSMode->setCurrentIndex(1); // TLS-Auth
        m_ui->kurlTlsAuthKey->setUrl(QUrl::fromLocalFile(wireguardKeyTa));
